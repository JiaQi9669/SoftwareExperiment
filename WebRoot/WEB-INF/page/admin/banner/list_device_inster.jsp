...
<body>
	<div class="place">
		...
		<ul class="placeul">
			...
			<li><a href="Managedeviceupdate">实验仪器</a>
			</li>
			<li><a href="#">修改详情</a>
			</li>
		</ul>
	...
	<s:fielderror></s:fielderror>
		<form action="Managedeviceinsterwrite?id=${device.id }" method="post"
			...
			<center style="font-size: 20px;_zoom:1;overflow:hidden;">
				<div style="float:left; position:relative;top:70px;left:300px;">
					请添加标题：<input type="text"
						...
						name="titles" value="${device.titles }"/>
				</div>
				<div class="file-box" style="float:left;left:400px;">
					<img id="file_img" style="height:150px; width:120px;"
						src="${pageContext.request.contextPath}/uploads/files/${device.path }">
					<input type="file" name="file" class="file" id="fileField"
						size="28" onchange="previewImage(this)" />
				</div>
			</center>
			<script name="context" id="editor" type="text/plain"
				style="width:1024px;height:500px;">${device.context }</script>
			<input class="submitBtn" type="submit" name="提交" />
		</form>
<%-- 	<form action="Managedeviceinsterwrite?id=${device.id }" method="post" enctype="multipart/form-data">
		<center style="font-size: 20px;_zoom:1;overflow:hidden;">
		<div style="float:left; position:relative;top:70px;left:300px;">请添加标题：
		<input type="text" style="font-size: 20px;font-style: !important;background-color:#cceeff;" name="titles" value="${device.titles }"/>
		</div>
		<div class="file-box" style="float:left;left:400px;">
		</div>
		</center>
		<script name="context" id="editor" type="text/plain" style="width:1024px;height:500px;">${device.context }</script>
		<input class="submitBtn" type="submit" name="提交"/>
	</form> --%>
	</center>
